...
Generate solvers and test drivers for
    Jacobi iteration; notes from 01/28 lecture
    Gauss Seidel Iteration; pseudocode on p.306 of textbook pdf
    SOR (SSOR is not necessary).
...
*/
    void randFillVec(std::chrono::high_resolution_clock::time_point t, double *a, int n)
    {
        ...
        for(int i=0; i<n; ++i)
        {
            a[i] =  double( distribution(generator) ) / 345678.7;
        }
    }
...
int main()
{
    ...
    x = new double[n];
    //cout << "\nGenerated x[] = " << endl;
    randFillVec( start, x, n); //randomly initialize x[]
    printVec("x", x, n);
